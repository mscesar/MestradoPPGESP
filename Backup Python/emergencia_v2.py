...
df = pd.read_excel('TB_IND_ASSIST_EME_0115_0319.xlsx')
...
#Training X Data Dimensions: (59012, 379)
#Training Y Data Dimensions: (59012,)
#Testing X Data Dimensions: (25291, 379)
...
cross_val_score_classifier = pd.ExcelWriter('cross_val_score_classifier_X_train_Y_train_kfold.xlsx', engine = 'xlsxwriter')
...
# Create a Pandas Excel Writer Using XlsxWriter as the Engine.
cross_val_score_classifier = pd.ExcelWriter('cross_val_score_classifier_X_Y_kfold.xlsx', engine = 'xlsxwriter')
# Convert the Dataframe to an XlsxWriter Excel Object.
kf_cross_val.to_excel(cross_val_score_classifier, sheet_name = 'Dados')
# Close the Pandas Excel Writer and Output the Excel File.
cross_val_score_classifier.save()
...
# Create a Pandas Excel writer using XlsxWriter as the engine.
cross_validate_classifier = pd.ExcelWriter('cross_validate_classifier_X_Y_kfold.xlsx', engine = 'xlsxwriter')
# Convert the dataframe to an XlsxWriter Excel object.
kf_cross_validate.to_excel(cross_validate_classifier, sheet_name = 'Dados')
# Close the Pandas Excel writer and output the Excel file.
cross_validate_classifier.save()
...
param_range_vc = [500, 1000, 2000, 3000, 4000, 5000]
...
feature_importances_lgbm_df.head(282)
...
score_lgbm, permutation_scores_lgbm, pvalue_lgbm = permutation_test_score(
    classifier_lgbm, X_train, Y_train, scoring = make_scorer(matthews_corrcoef), cv = skf_lgbm, n_permutations = 100 )
